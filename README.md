...
These `field_*()` methods are generated by `#[model]` and are the supported
way to obtain `FieldRef` values. The safe manual `FieldRef::new` constructor
was removed in 0.4.0 because an arbitrary field name or value type could forge
an invalid typed reference. For dynamic field names, validate against model
metadata and use a lower-level query API instead of constructing a `FieldRef`.
