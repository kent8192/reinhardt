...
fn cargo_process_command_line() -> Option<String> {
    let script = format!(
        "$process = Get-CimInstance Win32_Process -Filter 'ProcessId = {}'; if ($null -ne $process) {% raw %}{{ (Get-CimInstance Win32_Process -Filter \"ProcessId = $($process.ParentProcessId)\").CommandLine }}{% endraw %}",
        std::process::id()
    );
