...
fn generate_wrapper_with_both(
	...
	// Forward raw requests, extractors, and injected values in declaration order.
	...
	let call_args: Vec<_> = original_fn
		...
		.collect();
	let handler_call = quote! { #original_fn_name(#(#call_args),*).await };
...
mod url_resolver_tests {
	...
	#[rstest]
	fn route_call_passes_reinhardt_request_with_extractors_in_original_order() {
	...
	fn route_call_preserves_interleaved_argument_order() {
		...
		let generated = wrapper.to_string();
		assert!(generated.contains("handler_original (__reinhardt_extractor_0 , __reinhardt_inject_0 , __reinhardt_extractor_1 , __reinhardt_inject_1)"), "{generated}");
	}
	...
	fn route_call_passes_raw_request_with_extractors_in_original_order() {
		...
		assert!(
			generated.contains(
				"handler_original (__reinhardt_request , __reinhardt_extractor_0 , __reinhardt_extractor_1)"
			),
			"{generated}"
		);
	}
