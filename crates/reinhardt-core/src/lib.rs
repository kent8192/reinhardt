...
/// Re-export an endpoint function and its URL resolver module from a submodule.
///
/// When using multi-file view modules (each endpoint in its own file under
/// `views/`), the URL resolver module generated by `#[get]`/`#[post]`/etc.
/// lives inside the submodule. This macro re-exports both the function and
/// the resolver module so that `#[url_patterns]` can discover resolvers
/// using the standard parent-module path.
///
/// # Usage
///
/// ```rust,ignore
...
/// pub mod register;
///
/// reinhardt::use_endpoint!(login, login);
/// reinhardt::use_endpoint!(register, register);
/// // Expands to:
/// //   pub use login::login;
/// //   pub use login::__url_resolver_login;
/// ```
///
/// Then in `urls.rs`, the standard `.endpoint(views::login)` syntax works:
///
/// ```rust,ignore
/// #[url_patterns("auth")]
...
/// # Parameters
///
/// - `$module` — The submodule name (e.g., `login`)
/// - `$fn_name` — The endpoint function name (e.g., `login`)
///
/// For single-file views where the function is defined directly in `views.rs`,
/// this macro is not needed — use `.endpoint(views::login)` directly.
#[macro_export]
macro_rules! use_endpoint {
	($module:ident, $fn_name:ident) => {
		pub use $module::$fn_name;
		::paste::paste! {
			pub use $module::[<__url_resolver_ $fn_name>];
		}
	};
}
