...
impl FilesystemRepository {
	...
	pub fn render(&self, migration: &Migration, options: MigrationRenderOptions) -> Result<String> {
		...
		// Build the migration through the stable constructor and builders. This
		// keeps generated sources independent of the public struct layout.
		...
		let mut migration_builder = quote! { Migration::new(#name, #app_label) };
		for operation in &migration.operations {
			migration_builder.extend(quote! { .add_operation(#operation) });
		}
		for (dependency_app, dependency_name) in &migration.dependencies {
			migration_builder.extend(quote! { .add_dependency(#dependency_app, #dependency_name) });
		}
		for (replacement_app, replacement_name) in &migration.replaces {
			migration_builder
				.extend(quote! { .add_replacement(#replacement_app, #replacement_name) });
		}
		migration_builder.extend(quote! {
			.atomic(#atomic)
			.with_initial(#initial_tokens)
			.state_only(#state_only)
			.database_only(#database_only)
		});
		for dependency in swappable_dependencies {
			migration_builder.extend(quote! { .add_swappable_dependency(#dependency) });
		}
		for dependency in optional_dependencies {
			migration_builder.extend(quote! { .add_optional_dependency(#dependency) });
		}
