...
pub trait Model: Serialize + for<'de> Deserialize<'de> + Send + Sync + Clone {
	...
	/// The model's columns as typed bind values, each converted through its
	/// field type's [`DbValue`](super::DbValue). This is how the persistence layer
	/// gets a model's values without erasing their types through serde. The
	/// `#[model]` macro generates it; a hand-written `Model` impl that is actually
	/// persisted must override it (the default fails loudly rather than silently
	/// writing nothing).
	fn to_db_columns(&self) -> Vec<(&'static str, super::QueryValue)> {
		unimplemented!(
			"to_db_columns is generated by #[model]; a manual Model impl must \
			 override it to be persisted"
		)
	}

	/// Reconstruct the model from a database row: each column via its field
	/// type's [`DbValue`](super::DbValue), non-column fields (relationship
	/// accessors, m2m) defaulted — the read mirror of `to_db_columns`, analogous
	/// to Django's `Model.from_db`. The `#[model]` macro generates it; a
	/// hand-written `Model` impl that is read back must override it.
	fn from_db_columns(row: &super::connection::QueryRow) -> reinhardt_core::exception::Result<Self>
	where
		Self: Sized,
	{
		let _ = row;
		unimplemented!(
			"from_db_columns is generated by #[model]; a manual Model impl must \
			 override it to be hydrated"
		)
	}
