	...
	fs::write(
		...
		format!(
			"[package]\nname = \"rendered-migration-check\"\nversion = \"0.0.0\"\nedition = \
			 ...
			 when tinyvec publishes a release that compiles with the `alloc` feature without \
			 `std`.\n#\n# Ideal implementation (without workaround):\n# omit this direct pin \
			 and let isolated fixtures resolve tinyvec from crates.io.\ntinyvec = \
			 \"=1.12.0\"\n",
			workspace_root
		),
	)
