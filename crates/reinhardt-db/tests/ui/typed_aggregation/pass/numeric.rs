...
fn main() {
	...
	let _: TypedExpression<ModelRecord, i64, AggregateKind> = func::count(ModelRecord::field_i32());
	let _: TypedExpression<ModelRecord, i64, AggregateKind> = func::sum(ModelRecord::field_i32());
	...
	let _: TypedPredicate<ModelRecord> = scalar.eq(7_i64).unwrap();
	let _: HavingPredicate<ModelRecord> = func::sum(ModelRecord::field_i32()).gt(7_i64).unwrap();
}
