...
mod native {
	...
	fn initial_html_covers_all_stable_field_types_and_widgets(
		...
		// Act.
		let html = form.into_page().render_to_string();
	...
	fn collection_html_preserves_initial_selected_checked_and_formatted_values() {
		...
		assert_html_value(
			&form.into_page().render_to_string(),
			"rows_0_title",
			"text",
			"Ada",
		);
...
mod browser {
	...
	fn initial_dom_covers_all_stable_field_types_and_widgets() {
		let form = value_sync_form!();
		let mounted = MountedForm::new(form.into_page());
	...
	async fn native_reset_snapshots_shared_checkbox_values_before_writing_signals() {
		...
		let inputs = ["rust", "wasm"].map(|value| {
			...
			PageElement::new("input")
				...
				.control_binding(ControlBinding::from_parts(
					ControlKind::Checkbox,
					move || ControlValue::Checked(read.get().iter().any(|item| item == value)),
					move |raw| {
	...
	async fn cancelled_native_reset_keeps_the_current_source_and_dom() {
		let form = value_sync_form!();
		let mounted = MountedForm::new(form.clone().into_page());
		let element = reinhardt_pages::dom::Element::new(mounted.0.first_element_child().unwrap());
		let _cancel_reset =
			...
			.child(
				PageElement::new("select")
					...
					.control_binding(ControlBinding::from_parts(
						...
						move |raw| {
							let ControlValue::Text(value) = raw else {
								panic!("single select value")
							};
							write.set(value);
						},
					)),
	...
	fn file_signals_never_inject_nonempty_values_and_none_or_reset_clears_selection() {
		...
		let replacement = web_sys::File::new_with_str_sequence(&parts, "replacement.txt").unwrap();
