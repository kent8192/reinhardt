...
async fn search_backend_adds_where_clause_for_single_field() {
	...
	assert_eq!(
		result,
		"SELECT * FROM articles WHERE (`title` LIKE '%rust%' ESCAPE 0x5C)"
	);
}
...
async fn search_backend_generates_or_clause_for_multiple_fields() {
	...
	assert_eq!(
		result,
		"SELECT * FROM articles WHERE ((`title` LIKE '%web%' ESCAPE 0x5C OR `content` LIKE '%web%' ESCAPE 0x5C))"
	);
}
...
async fn search_backend_uses_mysql_dialect_by_default() {
	...
	assert_eq!(
		result,
		"SELECT * FROM products WHERE (`name` LIKE '%test%' ESCAPE 0x5C)"
	);
}
