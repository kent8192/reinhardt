...
mod tests {
	...
	#[test]
	fn message_mixin_delegates_tag_matching_and_stack_trace_modes() {
		// Arrange
		...
		let missing = preserving.assert_messages_with_tags(
			&messages,
			&[(
				Level::Success,
				"Profile saved".to_string(),
		...
		// Assert
		exists.expect("matching message should be found");
		ordered.expect("ordered message should match");
		tagged.expect("matching message tags should be found");
		match missing {
			Err(MessageAssertionError::MessageNotFound { message }) => assert_eq!(
				message,
				"Message with level Success, text 'Profile saved', and tags [\"other\"] not found"
			),
			Err(other) => panic!("unexpected message assertion error: {other}"),
			Ok(()) => panic!("expected mismatched tags to be rejected"),
		}
