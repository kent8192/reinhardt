...
mod tests {
	...
	fn server_fn_context_builds_overrides_auth_headers_and_expected_results() {
		...
		// Arrange
		let singleton = Arc::new(SingletonScope::new());
		let request = MockHttpRequest::post("/api/orders?dry_run=true")
			.with_cookie("session", "request-cookie");
		...
		assert_eq!(env.is_authenticated(), true);
		assert_eq!(env.user_id().is_some(), true);
		assert_eq!(env.has_permission("orders:read"), true);
		assert_eq!(env.has_permission("orders:delete"), false);
		assert_eq!(env.has_role("operator"), true);
		assert_eq!(env.has_role("admin"), false);
